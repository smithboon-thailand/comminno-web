...
/* eslint-disable */
/**
 * build-content.legacy.mjs — ARCHIVED Phase 1 content pipeline.
 *
 * ⚠️  DO NOT INVOKE THIS SCRIPT.  Kept on disk through commit #9 (Phase 2
 *     Sveltia /admin GA + ~1 week of stable editorial workflow), then
 *     deleted.  The active pipeline is `scripts/build-content.mjs`, which
 *     reads canonical YAML under `content/` and emits the same
 *     `client/src/content/*.ts` modules.
 *
 * Original purpose: parsed markdown sources from
 *   /home/ubuntu/comminno_assets/content/
 * and emitted typed TS modules under client/src/content/.  Superseded by
 * the YAML-as-source-of-truth model committed in Phase 2 r3 §2.
 *
 * The body below is preserved verbatim so editorial debugging can
 * still cross-reference the original parsing rules during the migration
 * cool-down period.  After deletion, recover from git history if needed.
 */
/**
